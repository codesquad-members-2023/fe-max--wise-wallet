...
## 📄 미리보기
![wise-wallet-demo](https://user-images.githubusercontent.com/76121068/227597365-8183a328-18bb-4f01-98e5-7400eefaa1d8.gif)

## 🧐 미션 회고

첫 미션이었던 3주 동안의 가계부 미션도 끝이 났다. 처음 기획서를 보았을 때만 해도 충분히 다 만들 수 있을 것이라 착각했지만 나의 오산이었다.
미션 1주차는 거의 html, css를 공부하고 기본적인 레이아웃을 짜면서 보냈었고, 2주차는 DOM, Event에 대해 학습하면서 메인화면의 인풋바의 드롭다운도 만들어보고
인풋바에 필요한 여러 기능들을 만들면서 보냈던 것 같다. 마지막 3주차는 입력한 데이터를 로컬 스토리지에 저장하고 메인화면에 렌더링하는 기능들을 작업했다.

결과적으로 수입/지출 내역이 추가만 되고 수정과 삭제는 불가능하고 월 이동 기능도 없는 반쪽짜리 가계부가 되어버렸다.
캘린더와 차트 영역은 진작에 포기하고 메인화면이라도 정상적으로 작동하는 것을 목표로 삼았는데, 그마저도 제대로 마무리 짓지 못해 너무 아쉽다.
그리고 특히나 마지막 3주차는 거의 코드 구현만 하다보니 루카스 학습스택이나 리드미나 노션에 학습 정리를 제대로 올리지 못했다.

중간중간 코어 타임에 집중해서 구현하지 못했던 때도 있었고 생활패턴이 들쭉날쭉 해지면서 컨디션도 기복이 좀 있었던 것 같다.
또 주말에도 열심히 작업하신 분들도 계셨는데 1,2주차 주말에 여러 일정들이 겹쳐 시간을 많이 투자하지 못했던 자신을 반성하게 된다.
마지막 주말을 활용해서 메인 화면의 남은 기능들을 작업해보려고 하는데 할 수 있는데까지는 최대한 만들어봐야 후회가 남지 않을 것 같다.

개인적으로 꽤나 어려운 미션이었다고 생각하는데, 그만큼 만드는 과정에서 새롭게 배운 것들도 많고 스스로 부족한 점이 무엇인지 좀 깨닫게 되었다.
본 과정에 들어와서 첫번째 미션이었던 만큼 아직은 미숙할 수 있다고 스스로 생각하면서 위안을 삼아본다.
...
- [ ]  메인 영역
    - [x]  로컬스토리지에 입력한 내역 데이터 저장하기
        - [x]  데이터 저장 형식 정하기
        - [x]  각 데이터 구분방식 생각하기
    - [x]  입력한 날짜별 내역 화면에 렌더링하기
        - [x]  새로운 submit 이벤트 발생할 때마다 새로운 항목을 추가하고 렌더링한다.
        - [x]  기존에 저장된 날짜가 아닌 새로운 날짜의 내역이 입력되면 새로운 daily-list 생성
            - [x]  기존에 있던 날짜인지 확인하기
            - [x]  새로운 daily-list 생성하기
        - [x]  해당하는 날짜의 daily-list에 daily-detail-list를 추가
            ...
            - [x]  daily-info 내용을 업데이트 한다.
        - [x]  전체내역 info-filter 내용 업데이트
    - [ ]  선택한 내역 수정 / 삭제 기능
    - [x]  전체내역 수입/지출 필터링 기능
